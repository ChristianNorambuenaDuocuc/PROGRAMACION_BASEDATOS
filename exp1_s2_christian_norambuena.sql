...
-- HABILITACIÓN SALIDAS POR PANTALLA
SET SERVEROUTPUT ON
...
DECLARE
  -- CURSOR QUE CONSIDERE EMPLEADOS DEL 100 AL 320
   CURSOR cr_usuario_clave IS
   -- Se realiza la selección de los datos brutos, necesarios para generar 
   -- el cursor.
    SELECT b.nombre_estado_civil,a.pnombre_emp, a.sueldo_base, a.dvrun_emp,
    a.numrun_emp,a.fecha_contrato ,a.fecha_nac, a.appaterno_emp,a.id_emp,a.apmaterno_emp,a.snombre_emp
    FROM EMPLEADO a
    JOIN ESTADO_CIVIL b ON a.id_estado_civil=b.id_estado_civil
    ORDER BY a.id_emp;
    
  -- SE DEFINEN VARIABLES DEL CURSOR cr_usuario_clave
  ...
  -- VARIABLES PARA DETERMINACION DE LOS USUARIO Y LAS CLAVE 
  v_1_letra_EC VARCHAR2(30);
  v_codigo_usu VARCHAR2(30);
  v_codigo_cla VARCHAR2(30);
  v_3_letra_PN VARCHAR2(30);
  v_largo_nombre NUMBER;
  v_asterisco CHAR :='*';
  v_total_registros NUMBER;
  v_an_nac_EMP NUMBER;
  v_ult_3_SB NUMBER;
  v_2_lt_APP VARCHAR2(30);
  v_ma_BD NUMBER;
  v_nombre_completo_EMP VARCHAR2(100);
  v_ult_num_SB NUMBER;
  v_an_empr NUMBER;
  v_x_10an CHAR(1);
  v_3_run_EMP NUMBER;
  v_contador NUMBER :=0;
  v_fecha DATE := TO_DATE(:b_fecha,'DDMMYYYY');

BEGIN
  -- CONTABILIZAR LA CANTIDAD DE REGISTROS(para realizar confirmaciòn de COMMIT)
  SELECT COUNT(*)
  INTO v_total_registros
  FROM empleado;
  
  --ABRIR CURSOR
  OPEN cr_usuario_clave;

  --RECORRIDO CON LOOP
  LOOP FETCH cr_usuario_clave INTO v_nombre_estado_civil,
  v_pnombre_emp, 
  v_sueldo_base,
  v_dvrun_emp, 
  v_numrun_emp,
  ...
  -- CONDICION DE SALIDA 
  EXIT WHEN cr_usuario_clave%NOTFOUND;
  
  -- GENERACION NOMBRE EN PL/SQL
  v_1_letra_EC := LOWER(SUBSTR(v_nombre_estado_civil, 1, 1));
  v_3_letra_PN := UPPER(SUBSTR(v_pnombre_emp, 1, 3));
  v_largo_nombre := LENGTH(v_pnombre_emp);
  v_ult_num_SB := SUBSTR(v_sueldo_base, -1, 1);
  v_an_empr :=EXTRACT(YEAR FROM v_fecha) - EXTRACT(YEAR FROM v_fecha_contrato);
  IF v_an_empr<10 THEN
     v_x_10an :='X';
     ELSE v_x_10an :='';
     END IF;
     
  -- CONCATENADO DE LOS DATOS PARA GENERAR LA COLUMNA DE USUARIO.
  v_codigo_usu := v_1_letra_EC || v_3_letra_PN || v_largo_nombre||v_asterisco||v_ult_num_SB||v_dvrun_emp||v_an_empr||v_x_10an;
     
  -- GENERACION DE LA CLAVE EN PL/SQL
  v_3_run_EMP := SUBSTR( v_numrun_emp, 3, 1);
  v_an_nac_EMP := EXTRACT(YEAR FROM v_fecha_nac)+2;
  v_ult_3_SB := SUBSTR( v_sueldo_base, -3, 3);
  IF v_ult_3_SB = 0 THEN
    v_ult_3_SB := 999;
    ELSE
    v_ult_3_SB := v_ult_3_SB - 1;
    END IF;
  -- CONDICIONAL, PARA DETERMINAR LOS VALORES DE v_2_lt_APP, DE ACUERDO AL TIPO DEL ESTADO CIVIL.
  IF UPPER(v_nombre_estado_civil)= 'CASADO' OR UPPER(v_nombre_estado_civil)= 'ACUERDO DE UNION CIVIL' THEN
      v_2_lt_APP :=LOWER(SUBSTR(v_appaterno_emp, 1, 2));
      ELSIF UPPER(v_nombre_estado_civil)= 'DIVORCIADO' OR UPPER(v_nombre_estado_civil)= 'SOLTERO' THEN
      v_2_lt_APP :=LOWER(SUBSTR(v_appaterno_emp, 1, 1))||LOWER(SUBSTR(v_appaterno_emp, -1, 1));
      ELSIF UPPER(v_nombre_estado_civil)= 'VIUDO' THEN
      v_2_lt_APP :=LOWER(SUBSTR(v_appaterno_emp, -3, 1))||LOWER(SUBSTR(v_appaterno_emp, -2, 1));
      ELSIF UPPER(v_nombre_estado_civil)= 'SEPARADO' THEN
      v_2_lt_APP :=LOWER(SUBSTR(v_appaterno_emp, -2, 1))||LOWER(SUBSTR(v_appaterno_emp, -1, 1));
      END IF;
      
  v_ma_BD :=EXTRACT(MONTH FROM v_fecha)||EXTRACT(YEAR FROM v_fecha); -- LA FECHA DE LA BASE DE DATOS NO APARECE EN LA INFORMACION DEL CASO, SE USAN LOS DATOS DEL SYSDATE.
     
  -- CONCATENADO DE LOS DATOS PARA GENERAR LA COLUMNA DE CLAVES.     
  v_codigo_cla := v_3_run_EMP || v_an_nac_EMP || v_ult_3_SB|| v_2_lt_APP||v_id_emp ||v_ma_BD;
      
  -- NOMBRE EMPLEADO(SE CONCATENA LOS NOMBRES Y APELLIDOS DE LOS EMPLEADOS) PARA INSERTAR LA COLUMNA NOMBRE_EMPLEADO EN USUARIO_CLAVE
  v_nombre_completo_EMP := UPPER(v_pnombre_emp)||' '||UPPER(v_snombre_emp)||' '||UPPER(v_appaterno_emp)||' '||UPPER(v_apmaterno_emp);
    
  -- REVISION EN PANTALLA DE LOS RESULTADOS
  DBMS_OUTPUT.PUT_LINE('El usuario '|| v_codigo_usu);
  DBMS_OUTPUT.PUT_LINE('El código '|| v_codigo_cla);
  DBMS_OUTPUT.PUT_LINE('contador '|| v_contador);
  DBMS_OUTPUT.PUT_LINE('FECHA BASE DATOS '|| v_ma_BD);
  DBMS_OUTPUT.PUT_LINE('');
      
  -- INSERTAR DATOS CONSTRUIDOS DE ACUERDO A PL/SQL A LA TABLA USUARIO CLAVE
  INSERT INTO USUARIO_CLAVE (ID_EMP,NUMRUN_EMP,DVRUN_EMP,NOMBRE_EMPLEADO,NOMBRE_USUARIO,CLAVE_USUARIO)
  VALUES (v_id_emp,v_numrun_emp,v_dvrun_emp,v_nombre_completo_EMP,v_codigo_usu,v_codigo_cla);
  
  -- SE GENERA CONTADOR PARA PODER VALIDAR LA CANTIDAD DE LOOP REALIZADOS  
  v_contador := v_contador +1;
  END LOOP;
  
  -- LIBERAR MEMORIA CERRANDO CURSOR
  CLOSE cr_usuario_clave;
 
  --CONTROLAR LAS TRANSACCIONES A TRAVES DE COMMIT Y ROLLBACK
  IF v_contador = v_total_registros THEN
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('INGRESO DE LOS DATOS CORRECTAMENTE');
    DBMS_OUTPUT.PUT_LINE('--------------------------------------------------');
    ELSE
    ROLLBACK;
    DBMS_OUTPUT.PUT_LINE('SE REALIZA ROLLBACK');
    DBMS_OUTPUT.PUT_LINE('--------------------------------------------------');
    END IF;
